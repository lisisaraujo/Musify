...
@Composable
fun PlayingSongScreen(viewModel: SongPlayViewModel = viewModel()) {
    ...
    LaunchedEffect(isPlaying, currentSong) {
        while (isPlaying && currentPosition < currentSong.duration) {
            delay(1000)
            currentPosition = viewModel.updatePosition(currentPosition, isPlaying, currentSong.duration)
        }
    ...
    Surface(
        ...
        Column {
            SongTopAppBar(artistName = currentSong.artist)
            SongDetailCardView(currentSong)
            ...
            PlaybackControls(
                onPreviousClick = {
                    currentSongIndex = viewModel.getPreviousSongIndex(currentSongIndex, allSongs.size)
                    currentPosition = 0
                },
